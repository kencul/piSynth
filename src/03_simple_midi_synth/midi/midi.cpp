#include "midi.hpp"
#include "../config.hpp"
#include <cmath>
#include <iostream>
#include <poll.h>
#include <string>
#include <vector>

MidiReader::MidiReader(std::atomic<double> &frequency, std::atomic<unsigned int> &notes_active) :
    frequency(frequency), notes_active(notes_active) {}

MidiReader::~MidiReader() { stop(); }

bool MidiReader::open(const char *device_name) {
	if (snd_seq_open(&seq, "default", SND_SEQ_OPEN_DUPLEX, 0) < 0) {
		std::cerr << "MidiReader: failed to open sequencer\n";
		return false;
	}

	snd_seq_set_client_name(seq, "Midi Synth");

	in_port = snd_seq_create_simple_port(seq,
	                                     "MIDI In",
	                                     SND_SEQ_PORT_CAP_WRITE | SND_SEQ_PORT_CAP_SUBS_WRITE,
	                                     SND_SEQ_PORT_TYPE_APPLICATION);
	if (in_port < 0) {
		std::cerr << "MidiReader: failed to create input port\n";
		return false;
	}

	int src_client = find_client(device_name);
	if (src_client < 0) {
		std::cerr << "MidiReader: could not find device: " << device_name << "\n";
		return false;
	}

	snd_seq_connect_from(seq, in_port, src_client, 0);

	std::cout << "MidiReader: connected to " << device_name << " (client " << src_client << ":0)\n";
	return true;
}

void MidiReader::start() {
	running.store(true);
	thread = std::thread(&MidiReader::midi_loop, this);
}

void MidiReader::stop() {
	running.store(false);
	if (thread.joinable()) thread.join();

	if (seq) {
		snd_seq_close(seq);
		seq = nullptr;
	}
}

void MidiReader::midi_loop() {
	// get the sequencer's file descriptors for polling
	int npfds = snd_seq_poll_descriptors_count(seq, POLLIN);
	std::vector<pollfd> pfds(npfds);
	snd_seq_poll_descriptors(seq, pfds.data(), npfds, POLLIN);

	while (running.load()) {
		// block for up to 50ms — then wake up and check running flag
		int ret = poll(pfds.data(), npfds, 50);

		if (ret < 0) {
			if (errno == EINTR) continue; // signal interrupted poll, loop and recheck flag
			std::cerr << "MidiReader: poll error\n";
			break;
		}

		if (ret == 0) continue; // timeout — loop back and check running.load()

		// drain all pending events in one shot
		while (snd_seq_event_input_pending(seq, 1) > 0) {
			snd_seq_event_t *ev;
			if (snd_seq_event_input(seq, &ev) < 0) break;
			handle_event(ev);
		}
	}
}

int MidiReader::find_client(const char *search) {
	snd_seq_client_info_t *info;
	snd_seq_client_info_alloca(&info);
	snd_seq_client_info_set_client(info, -1);

	while (snd_seq_query_next_client(seq, info) >= 0) {
		const char *name = snd_seq_client_info_get_name(info);
		if (strstr(name, search)) return snd_seq_client_info_get_client(info);
	}
	return -1;
}

void MidiReader::handle_event(snd_seq_event_t *ev) {
	switch (ev->type) {
		case SND_SEQ_EVENT_NOTEON:
			if (ev->data.note.velocity > 0) {
				frequency.store(midi_to_hz(ev->data.note.note));
				notes_active.fetch_add(1);
				std::cout << "Note On  | " << note_name(ev->data.note.note) << " ("
				          << (int)ev->data.note.note << ")"
				          << " vel=" << (int)ev->data.note.velocity << "\n";
			} else {
				if (notes_active.load() > 0) notes_active.fetch_sub(1);
			}
			break;

		case SND_SEQ_EVENT_NOTEOFF:
			if (notes_active.load() > 0) notes_active.fetch_sub(1);
			std::cout << "Note Off | " << note_name(ev->data.note.note) << "\n";
			break;

		case SND_SEQ_EVENT_CONTROLLER:
			std::cout << "CC       | cc=" << (int)ev->data.control.param
			          << " val=" << (int)ev->data.control.value << "\n";
			break;

		default: break;
	}
}

double MidiReader::midi_to_hz(int note) {
	// A4 = note 69 = 440Hz, equal temperament
	return 440.0 * std::pow(2.0, (note - 69) / 12.0);
}

std::string MidiReader::note_name(int note) {
	static const char *names[] = {"C", "C#", "D", "D#", "E", "F", "F#", "G", "G#", "A", "A#", "B"};
	return std::string(names[note % 12]) + std::to_string((note / 12) - 1);
}