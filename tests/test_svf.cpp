...
static float measure_gain(float cutoff_hz, float resonance, float test_freq_hz) {
	...
	const float inc = 2.0f * std::numbers::pi_v<float> * test_freq_hz / float(Config::SAMPLE_RATE);
	float phase     = 0.0f;
...
static float to_db(float linear_gain) { return 20.0f * std::log10(linear_gain); }
