...
static float measure_pluck_frequency(float target_hz) {
    ...
    if (best_lag > min_lag && best_lag < max_lag) {
        ...
        if (std::abs(denom) > 1e-10f)
            refined_lag += 0.5f * (ym1 - yp1) / denom;
    }
...
TEST_CASE("Pluck frequency accuracy is within ±2 cents across MIDI range", "[pluck][tuning]") {
    struct Note { const char* name; float hz; };
    ...
    for (auto& [name, hz] : notes) {
        ...
        INFO(name << " (" << hz << " Hz):  measured=" << measured
...
// Test with combinations that keep feedback_gain below the 0.4995 cap.
// The cap engages for slow decays at high frequencies; A3 (220 Hz) stays
// clear of it across all three decay rates tested here.
//
// Tolerance is ±20% because the RMS includes harmonics that decay at different
// rates than the fundamental: slow decays (10 dB/sec) measure slightly fast
// (harmonics persist and fade unevenly across windows), while fast decays
// (30 dB/sec) measure slightly slow (window 2 is more purely fundamental than
// window 1, making the slope appear shallower than the true fundamental rate).
TEST_CASE("Pluck decay rate matches requested dB/sec", "[pluck][decay]") {
    struct Case { float freq_hz; float decay_db_per_sec; };
