...
template <typename T, typename Enable = void> struct is_std_string : types::false_type {};
...
struct is_std_string<
    ...
    typename types::enable_if<
        ...
        && types::is_same<decltype(declval<const T &>().size()), size_t>::value>::type> :
    types::true_type {};
...
public:
	...
	template <typename T,
	          typename detail::types::enable_if<detail::is_std_string<T>::value, bool>::type = true>
...
template <typename T>
struct is_pair<T,
               types::void_t<typename T::first_type,
                             typename T::second_type,
                             decltype(declval<T>().first),
                             decltype(declval<T>().second)>> : types::true_type {};

template <typename T, typename = void> struct is_container : types::false_type {};

template <typename T>
struct is_container<T,
                    types::void_t<typename T::value_type,
                                  typename T::iterator,
...
struct StringMaker :
    public detail::StringMakerBase<detail::has_insertion_operator<T>::value
                                   ...
                                   || detail::types::is_array<T>::value || detail::is_pair<T>::value
...
template <typename T>
struct filldata<T,
                typename detail::types::enable_if<!detail::has_insertion_operator<T>::value
...
struct DOCTEST_INTERFACE ResultBuilder : public AssertData {
	...
	template <typename L> DOCTEST_NOINLINE bool unary_assert(const DOCTEST_REF_WRAP(L) val) {
		...
		if (m_at & assertType::is_false) { m_failed = !m_failed; }

		if (m_failed || getContextOptions()->success) { m_decomp = (DOCTEST_STRINGIFY(val)); }
...
DOCTEST_MSVC_SUPPRESS_WARNING(4018) // 'expression' : signed/unsigned mismatch
			// DOCTEST_MSVC_SUPPRESS_WARNING(4805) // 'operation' : unsafe mix of type 'type' and
...
template <typename L> struct Expression_lhs {
	...
	DOCTEST_NOINLINE operator Result() {
		...
		if (!res || getContextOptions()->success) { return {res, (DOCTEST_STRINGIFY(lhs))}; }
		return {res};
	}
...
template <typename T> int registerExceptionTranslator(String (*translateFunction)(T)) noexcept {
...
template <typename T> int registerExceptionTranslator(String (*)(T)) noexcept { return 0; }
...
DOCTEST_INTERFACE bool decomp_assert(
    assertType::Enum at, const char *file, int line, const char *expr, const Result &result);
...
DOCTEST_NOINLINE bool unary_assert(assertType::Enum at,
	...
	if (at & assertType::is_false) failed = !failed;
...
template <typename T, typename... Rest> T acquireGeneratorValue(T first, Rest... rest) {
...
template <typename T> int instantiationHelper(const T &) noexcept { return 0; }
	...
	// if registering is not disabled
		...
		#define DOCTEST_ADD_MESSAGE_AT(file, line, ...) \
			DOCTEST_ADD_AT_IMPL( \
			    is_warn, file, line, DOCTEST_ANONYMOUS(DOCTEST_MESSAGE_), __VA_ARGS__)
		#define DOCTEST_ADD_FAIL_CHECK_AT(file, line, ...) \
			DOCTEST_ADD_AT_IMPL( \
			    is_check, file, line, DOCTEST_ANONYMOUS(DOCTEST_MESSAGE_), __VA_ARGS__)
		#define DOCTEST_ADD_FAIL_AT(file, line, ...) \
			DOCTEST_ADD_AT_IMPL( \
			    is_require, file, line, DOCTEST_ANONYMOUS(DOCTEST_MESSAGE_), __VA_ARGS__)
...
public:
	...
	T load(std::memory_order order = std::memory_order_seq_cst) const DOCTEST_NOEXCEPT {
		auto result = T();
		for (const auto &c : m_atomics) { result += c.atomic.load(order); }
		return result;
	}
...
void addAssert(assertType::Enum at) {
	if ((at & assertType::is_warn) == 0) g_cs->numAssertsCurrentTest_atomic++;
}

void addFailedAssert(assertType::Enum at) {
	if ((at & assertType::is_warn) == 0) g_cs->numAssertsFailedCurrentTest_atomic++;
}
...
void MessageBuilder::react() {
	if (m_severity & assertType::is_require) throwException();
}
...
const char *failureString(assertType::Enum at) {
	if (at & assertType::is_warn) return "WARNING";
	if (at & assertType::is_check) return "ERROR";
	if (at & assertType::is_require) return "FATAL ERROR";
	return "";
}
...
struct ConsoleReporter : public IReporter {
	...
	static const char *
	getSuccessOrFailString(bool success, assertType::Enum at, const char *success_str);
...
bool parseCommaSepArgs(int argc,
	...
	if (parseOption(argc, argv, pattern, &filtersString)) {
		...
		auto flush = [&s, &res]() {
			auto string = s.str();
			if (!string.empty()) { res.emplace_back(string.c_str()); }
			s.str("");
		};
...
bool parseIntOption(
    int argc, const char *const *argv, const char *pattern, optionType type, int &res) {
	String parsedValue;
...
Context::~Context() {
	if (detail::g_cs == p) detail::g_cs = nullptr;
	delete p;
}
...
void Context::parseArgs(int argc, const char *const *argv, bool withDefaults) {
	...
	#define DOCTEST_PARSE_AS_BOOL_OR_FLAG(name, sname, var, default) \
		if (parseIntOption( \
		        argc, argv, DOCTEST_CONFIG_OPTIONS_PREFIX name "=", option_bool, intRes) \
		    || parseIntOption( \
		        argc, argv, DOCTEST_CONFIG_OPTIONS_PREFIX sname "=", option_bool, intRes)) \
	...
	#define DOCTEST_PARSE_INT_OPTION(name, sname, var, default) \
		if (parseIntOption(argc, argv, DOCTEST_CONFIG_OPTIONS_PREFIX name "=", option_int, intRes) \
		    || parseIntOption( \
		        argc, argv, DOCTEST_CONFIG_OPTIONS_PREFIX sname "=", option_int, intRes)) \
			p->var = intRes; \
		else if (withDefaults) \
		p->var = default
...
int Context::run() {
	...
	if (p->filters[8].empty()) p->filters[8].emplace_back("console");
	...
	for (auto &curr : testArray) {
		...
		// execute the test if it passes all the filtering
		{
	       ...
	       // static method)
				...
				if (!has_next_path) run_test = false;
			} while (run_test);
...
ContextScopeBase::ContextScopeBase(ContextScopeBase &&other) noexcept {
	if (other.need_to_destroy) { other.destroy(); }
...
bool checkIfShouldThrow(assertType::Enum at) {
	if (at & assertType::is_require) return true;
...
int IReporter::get_num_active_contexts() { return static_cast<int>(detail::g_infoContexts.size()); }
...
ConsoleReporter::ConsoleReporter(const ContextOptions &co) : s(*co.cout), opt(co) {}

ConsoleReporter::ConsoleReporter(const ContextOptions &co, std::ostream &ostr) : s(ostr), opt(co) {}
...
void ConsoleReporter::successOrFailColoredStringToStream(bool success,
                                                         assertType::Enum at,
                                                         const char *success_str) {
...
void ConsoleReporter::report_query(const QueryData &in) {
	if (opt.version) {
		...
		for (unsigned i = 0; i < in.num_data; ++i) s << Color::None << in.data[i]->m_name << "\n";
...
void ConsoleReporter::test_run_end(const TestRunStats &p) {
	...
	auto passwidth = static_cast<int>(std::ceil(log10(
	    ...
	    + 1)));
	auto failwidth = static_cast<int>(
	    std::ceil(log10(static_cast<double>(std::max(p.numTestCasesFailed,
...
JUnitReporter::JUnitTestCaseData::JUnitTestMessage::JUnitTestMessage(const std::string &_message,
                                                                     const std::string &_type,
                                                                     const std::string &_details) :
...
JUnitReporter::JUnitTestCaseData::JUnitTestCase::JUnitTestCase(const std::string &_classname,
...
JUnitReporter::JUnitReporter(const ContextOptions &co) : xml(*co.cout), opt(co) {}
...
void JUnitReporter::test_case_end(const CurrentTestCaseStats &st) {
	...
	if (st.failure_flags & TestCaseFailureReason::Timeout) {
		...
		*stream << "Test case exceeded time limit of " << std::setprecision(6) << std::fixed
		        << tc->m_timeout;
...
static_assert(DOCTEST_COUNTOF(signalDefs) == DOCTEST_COUNTOF(FatalConditionHandler::oldSigActions),
...
void FatalConditionHandler::allocateAltStackMem() { altStackMem = new char[altStackSize]; }
...
void String::setLast(size_type in) noexcept { buf[last] = static_cast<char>(in); }
...
Subcase::Subcase(const String &name, const char *file, int line) : m_signature({name, file, line}) {
	...
	if (!g_cs->traversal.tryEnterSubcase(m_signature)) return;
...
DOCTEST_NOINLINE DecisionPoint &TraversalState::ensureDecisionPointAtCurrentDepth() {
	...
	if (m_discoveredDecisionPath.size() == depth) {
		...
		if (m_decisionPath.size() == depth) m_decisionPath.push_back(0);
	}
...
bool TraversalState::tryEnterSubcase(const SubcaseSignature &signature) {
	...
	for (; siblingIndex < subcases.size(); ++siblingIndex) {
		if (subcases[siblingIndex] == signature) break;
	}

	if (siblingIndex == subcases.size()) subcases.push_back(signature);

	point.branch_count = subcases.size();

	if (siblingIndex != m_decisionPath[m_decisionDepth]) return false;
...
size_t TraversalState::unwindActiveSubcases() {
	...
	while (m_activeSubcaseDepth > 0) leaveSubcase();
